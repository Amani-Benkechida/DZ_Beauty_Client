...
const Cart = ({ cartItems = [], dateFromDB }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    cvv: '',
    expiration: '',
    postalCode: '',import React, { useState } from 'react';
import { useNavigate } from "react-router-dom";
const Cart = ({ cartItems = [], dateFromDB }) => {
  // Calculate the total amount
  const totalAmount = cartItems.reduce((total, item) => total + item.amount, 0);
  ...
  const [showOverlay, setShowOverlay] = useState(false); // Initially, overlay is hidden
      ...
      <div className="w-4/5 bg-white shadow-md">
        <div className="p-6">
          ...
          <div className="flex justify-between items-center w-full mb-6">
            ...
            <button className="h-[44px] bg-buttonColor text-white w-[184px] hover:bg-button_hover" onClick={() => navigate("/addservice")}>
              Add service
            </button>
          </div>
          ...
          <div className="mt-6">
            <button
              onClick={() => setShowOverlay(true)} // Always show overlay when clicked
